...
def get_decision_parameters(input_state, fields, maxpoints, n=5, k=6):
    ...
    propability = [[None] * 1 for _ in range(number_of_states)]
    reroll_dice_list = [[None] * 1 for _ in range(number_of_states)]
    for i, state in enumerate(states):
        number_of_reroll_dice, reroll_dice, number_of_permutations = compare_states(input_state, state)
        ...
        for field in fields:
            ...
            potential_points += currentpoints / (maxpoints[field-1]**1) * currentpoints
...
def get_decision(propability, reroll_dice):
    ...
    for i in range(len(unique_reroll_dice)):
        indices = [index for index, value in enumerate(reroll_dice) if value == unique_reroll_dice[i]]
...
maxpoints = [3, 6, 9]
propability, reroll_dice = get_decision_parameters((1, 2, 3), [1, 2, 3], maxpoints, 3, 3)
unique_reroll_dice, propscores = get_decision(propability, reroll_dice)
for i,j in enumerate(unique_reroll_dice):
    print(j, propscores[i])
