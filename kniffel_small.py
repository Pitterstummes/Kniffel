...
def init():
    text = ["Name", "Ones", "Twos", "Threes", "Fours", "Fives", "Sixes", "Sum", "Bonus", "Total 1",\
            "One pair", "Two pairs", "Three of a kind", "Four of a kind", "Full house", "Small street",\
            "Big street", "Kniffel", "Chance", "Total 2", "Total"] # Text for the scoreboard
...
def run_game(name, throws=3, n=5, k=6, printinfo=False):
    if printinfo:
    ...
    while len(freefields) > 0:
        ...
        scoreboard, freefields = update_score(scoreboard, freefields, freefields[index], int(current_points[index]))
    scoreboard = calculate_scores(scoreboard)    
    for i,j in enumerate(scoreboard):
        print(text[i]+"\t", j)

run_game("Test", printinfo=True)